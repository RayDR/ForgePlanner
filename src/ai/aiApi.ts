...
import type { AiOperationDto, AiProposalResult } from './aiTypes'

function cookie(name: string) { return document.cookie.split('; ').find((item) => item.startsWith(`${name}=`))?.split('=').slice(1).join('=') }
async function request<T>(path: string, init: RequestInit = {}) { const csrf = cookie('northstar_csrf'); const guestCsrf = cookie('northstar_ai_csrf'); const response = await fetch(`/api/ai${path}`, { ...init, credentials: 'include', headers: { ...(init.body ? { 'content-type': 'application/json' } : {}), ...(csrf ? { 'x-csrf-token': decodeURIComponent(csrf) } : {}), ...(guestCsrf ? { 'x-ai-guest-csrf': decodeURIComponent(guestCsrf) } : {}), ...init.headers } }); const body = await response.json().catch(() => null); if (!response.ok) throw new Error(body?.error?.message ?? 'AI proposal request failed.'); return body as T }
export const aiApi = {
  guestSession: () => request<{ expiresAt: string; csrfToken: string }>('/guest/session', { method: 'POST' }),
  generate: (input: object, guest: boolean, signal?: AbortSignal) => request<AiProposalResult>(guest ? '/guest/plan-proposals' : '/plan-proposals', { method: 'POST', body: JSON.stringify(input), signal }),
