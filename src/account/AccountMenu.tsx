...
export function AccountMenu() {
  ...
  const name = session?.user.profile?.displayName ?? session?.user.email ?? 'NorthStar'
  ...
  return <div className="account-menu" ref={ref}>
    <button type="button" className="account-avatar-button" aria-haspopup="menu" aria-expanded={open} onClick={() => setOpen((value) => !value)} title={name}>{initials}</button>
    {open ? <div className="account-menu-popover" role="menu">
      <header><span>{initials}</span><div><strong>{name}</strong><small>{session?.user.email}</small></div></header>
      <button role="menuitem" onClick={() => go('/account')}>◎ <span>{es ? 'Mi perfil' : 'My profile'}</span></button>
