...
export function PlanInvitations({ locale, onAccepted }: { locale: Locale; onAccepted: () => void }) {
  ...
  async function respond(id: string, response: 'accepted' | 'declined') { const scope = getIdentityScope(); const generation = getScopeGeneration(); if (!scope) return; try { await sharingApi.respond(id, response); if (!isCurrentScope(scope, generation)) return; await load(); if (response === 'accepted' && isCurrentScope(scope, generation)) onAccepted() } catch (reason) { if (isCurrentScope(scope, generation)) setError(reason instanceof Error ? reason.message : String(reason)) } }
  if (!session || (!items.length && !error)) return null
