...
const plan = {
  ...
  snapshot: { schemaVersion: 7, project: { id: 'document-id' }, activities: [], trash: [], relationships: [], selectedYear: 2026, selectedMonthId: '2026-01', locale: 'en', theme: 'dark' },
...
describe('plan API server-first creation', () => {
  ...
  it('posts a mutation ID without client synchronization or ownership metadata', async () => {
    ...
    expect(body).not.toHaveProperty('ownerUserId'); expect(body).not.toHaveProperty('syncState'); expect(body.snapshot).not.toHaveProperty('syncByPlanId')
  })
})
