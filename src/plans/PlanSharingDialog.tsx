  ...
  return <ModalPortal><div className="modal-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className="modal-shell plan-sharing-dialog" role="dialog" aria-modal="true" aria-labelledby="sharing-title">
      ...
      <div className="modal-body stack-sm">
        ...
        <section className="sharing-general-access"><header><div><ShareIcon width={20} /><span><strong>{t.linkTitle}</strong><small>{link?.enabled ? t.linkEnabled : t.linkDisabled}</small></span></div>{link ? <select className="field-input" value={link.accessLevel} disabled={!sharingEnabled} onChange={(event) => void updateLink({ accessLevel: event.target.value as 'viewer' | 'editor' })}><option value="viewer">{t.view}</option><option value="editor">{t.edit}</option></select> : null}</header>{link ? <><p className="sharing-link-warning">{link.accessLevel === 'editor' ? t.linkEditWarning : t.linkViewWarning}</p><div><button type="button" className="btn" disabled={!sharingEnabled} onClick={() => void updateLink({ enabled: !link.enabled })}>{link.enabled ? t.disableLink : t.enableLink}</button><button type="button" className="btn btn-primary" disabled={!sharingEnabled || !link.enabled} onClick={() => void copyLink()}>{t.copyLink}</button><button type="button" className="btn btn-danger" disabled={!sharingEnabled} onClick={() => void removeLink()}>{t.removeLink}</button></div></> : <button type="button" className="btn" disabled={!sharingEnabled} onClick={() => void createLink()}>{t.createLink}</button>}</section>
