...
export function planRoutes(db: PrismaClient) {
  ...
  router.post('/', csrfProtection, requirePermission(permissions.PLAN_CREATE), async (request, response) => response.status(201).json({ plan: await service.create(request.auth!.effectiveUserId, planPayloadSchema.parse(request.body), identity(request)) }))
