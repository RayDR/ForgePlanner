...
import { guestRefinementSchema, guestTransitionSchema, listSchema, proposalInputSchema, refinementSchema, revisionSchema, transitionSchema } from './ai.schemas.js'
...
export function aiProposalRoutes(db: PrismaClient, env: AppEnv) {
  const router = Router(); const provider = new MockAiProposalProvider(); const service = new AiProposalService(db, provider)
  ...
  router.post('/guest/plan-proposals', guestGenerationLimit, async (request, response) => { const controller = controllerFor(request); response.status(201).json(await guest!.generate(guestSession(request), proposalInputSchema.parse(request.body), controller.signal)) })
  ...
  router.post('/plan-proposals', generationLimit, csrfProtection, async (request, response) => { const controller = controllerFor(request); response.status(201).json(await service.generate(request.auth!.effectiveUserId, proposalInputSchema.parse(request.body), identity(request), controller.signal)) })
