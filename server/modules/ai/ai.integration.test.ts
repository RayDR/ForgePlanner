...
integration('PostgreSQL AI proposal lifecycle', () => {
  ...
  it('recovers abandoned leases and discards the stale provider result', async () => {
    let clock = new Date('2026-07-21T00:00:00Z'); const initial = new AiProposalService(db, new MockAiProposalProvider(), () => clock); const created = await initial.generate(userA.id, input(), identity(userA.id)); const provider = new GateProvider(); const service = new AiProposalService(db, provider, () => clock); const pending = service.refine(userA.id, created.operation.id, { clientRequestId: randomUUID(), expectedRevision: 1, instruction: 'Reduce budget' }, identity(userA.id)); await provider.startedPromise; clock = new Date('2026-07-21T00:01:00Z'); const cleanup = await new AiProposalExpirationService(db, () => clock).run({ limit: 10, dryRun: false }); expect(cleanup.refinementLeasesRecovered).toBe(1); provider.release(); await expect(pending).rejects.toMatchObject({ code: 'AI_PROPOSAL_CONFLICT' }); expect(await db.aiProposalRevision.count({ where: { aiOperationId: created.operation.id } })).toBe(1); expect((await db.aiOperation.findUniqueOrThrow({ where: { id: created.operation.id } })).status).toBe('PROPOSED')
  })
