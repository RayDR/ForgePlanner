...
export class AiProposalExpirationService {
  ...
  async run(input: { limit: number; dryRun: boolean }) {
    const now = this.now(); const result = { scanned: 0, generationLeasesRecovered: 0, refinementLeasesRecovered: 0, expired: 0, purged: 0, skipped: 0, failed: 0, dryRun: input.dryRun }
    const leases = await this.db.aiOperation.findMany({ where: { status: { in: ['PENDING','REFINING'] }, processingLeaseExpiresAt: { lte: now } }, orderBy: [{ processingLeaseExpiresAt: 'asc' }, { id: 'asc' }], take: input.limit, select: { id: true, status: true, processingRequestId: true } })
